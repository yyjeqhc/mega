...
impl DicItem {
    ...
    async fn get_type(&self) -> ContentType {
...
impl DictionaryStore {
    ...
    async fn update_inode(&self, parent: u64, item: Item) -> std::io::Result<u64> {
        ...
        if let Ok(pinode) = prw.get_item(parent) {
            ...
            println!(
                "radix_trie {:?}",
                GPath::from(item.path.clone()).to_string()
            );
...
pub async fn import_arc(store: Arc<DictionaryStore>) {
    ...
    for it in items {
        ...
        println!(
            "import_arc {:?} {:?} {:?} ",
            it.name, it.path, it.content_type
        );

        let it_inode = store.update_inode(1, it).await.unwrap();
        println!("node {:?}", it_inode);
        if is_dir {
            queue.push(it_inode);
        }
    }
                    ...
                    match fetch_tree(&path).await {
                        Ok(new_items) => {
                            ...
                            for newit in new_items {
                                ...
                                println!(
                                    "import_arc {:?} {:?} {:?}",
                                    newit.name, newit.path, newit.content_type
                                );

                                let new_inode = store.update_inode(inode, newit).await.unwrap();
                                println!("node {:?}", new_inode);
                                if is_dir {
    ...
    // tokio::spawn(update_dir(store.clone()));
}

pub async fn update_dir(store: Arc<DictionaryStore>) {
    loop {
        ...
        for it in items {
            ...
            let node = store.get_inode_from_path(&it.path).await;
            //alerady have
            if node.is_err() {
                let it_inode = store.update_inode(1, it).await.unwrap();
                if is_dir {
                    ...
                    if let Some(inode) = queue.pop() {
                        ...
                        match fetch_tree(&path).await {
                            Ok(new_items) => {
                                ...
                                for newit in new_items {
                                    let is_dir = newit.is_dir();
                                    println!("add {:?}", newit.path);

                                    let node = store.get_inode_from_path(&newit.path).await;
                                    if node.is_err() {
                                        let new_inode =
                                            store.update_inode(inode, newit).await.unwrap();
                                        if is_dir {
                                            // If it's a directory, push it to the queue and add the producer count
                                            producers.fetch_add(1, Ordering::Relaxed);
                                            queue.push(new_inode);
